...
def grid_unstr_sheet10(a, hmin, hmax, namestring):
# For ex. sheet 10:
# Produces an unstructured grid of a square domain Omega = (-a,a)² with
# a...sidelength of the square
# hmax...maximal mesh width at the corners
# hmin...maximal mesh width at points (-a,0) and (a,0)
# namestring is the string containing the file name to use (extensions '.geo' and '.msh' will be appended)
# Returns three arrays:
# p...an array of dimension Nx2 containing the plane coordinates of all nodes in the mesh 
# t...an array of dimension Mx3 containing all the indices of nodes of all triangles in the mesh
# be..an array of dimension Bx2 containing all the indices of all boundary edges in the mesh
   ...
   a_str = str(a);	# conversion to string
   hmax_str = str(hmax);	# converison to string
   hmin_str = str(hmin);	# conversion to string
   
   fid = open(namestring + '.geo','w');					# create a new .geo-file
   fid.write('Point(1) = {-'+a_str+', -'+a_str+', 0, '+ hmax_str +'};\n'); # write first point of square: lower left corner
   fid.write('Point(2) = { '+a_str+', -'+a_str+', 0, '+ hmax_str +'};\n'); # write second point of square (counter-clockwise order)...
   fid.write('Point(3) = { '+a_str+',  '+a_str+', 0, '+ hmax_str +'};\n');
   fid.write('Point(4) = {-'+a_str+',  '+a_str+', 0, '+ hmax_str +'};\n');
   fid.write('Point(5) = {-'+a_str+',  0, 0, '+ hmin_str +'};\n'); # write points where b.c. changes...
   fid.write('Point(7) = { '+a_str+',  0, 0, '+ hmin_str +'};\n');
   fid.write('Point(6) = {0, 0, 0, '+ hmax_str +'};\n'); # add origin
   fid.write('Line(1) = {1, 2};\nLine(2) = {2, 7};\nLine(3) = {7, 6};\n');
   fid.write('Line(4) = {6, 5};\nLine(5) = {5, 1};\n'); # define lines...
   fid.write('Line(6) = {7, 3};\nLine(7) = {3, 4};\nLine(8) = {4, 5};\n'); # between corners and singular nodes
   fid.write('Line Loop(1) = {1, 2, 3, 4, 5};\n')	# define line loop
   fid.write('Line Loop(2) = {-4,-3, 6, 7, 8};\n')	# define line loop   
   fid.write('Plane Surface(1) = {1};\n');	# define surface
   fid.write('Plane Surface(2) = {2};\n');	# define surface
   fid.close()					# close file
   
   subprocess.call(['gmsh', namestring + '.geo', '-2','-v', '3']);	# mesh .geo-file into msh.file,
   p, t, be = read_gmsh(namestring + '.msh');			# use read_gmsh to create output

   return p, t, be

def grid_unstr_sheet10B(bgmesh, namestring):
# For ex. sheet 10:
# Produces a graded mesh of a square domain Omega = (-1,1)² using a background mesh with
# bgmesh...number of background mesh to use (0..8)
# namestring is the string containing the file name to use (extensions '.geo' and '.msh' will be appended)
# Returns three arrays:
# p...an array of dimension Nx2 containing the plane coordinates of all nodes in the mesh 
# t...an array of dimension Mx3 containing all the indices of nodes of all triangles in the mesh
# be..an array of dimension Bx2 containing all the indices of all boundary edges in the mesh
   ...
   bgmesh=int(bgmesh);
   if (bgmesh<0 or bgmesh>8): error('number of background mesh out of bound!')
   
   bg_str = str(bgmesh);	# conversion to string
   
   #fid = open(namestring + '.geo','w');					# create a new .geo-file
   #fid.write('Point(1) = {-1, -1, 0};\n');
   #fid.write('Point(2) = { 1, -1, 0};\n');
   #fid.write('Point(3) = { 1,  1, 0};\n');
   #fid.write('Point(4) = {-1,  1, 0};\n');
   #fid.write('Line(1) = {1, 2};\nLine(2) = {2, 3};\nLine(3) = {3, 4};\nLine(4) = {4, 1};\n'); # define lines
   #fid.write('Line Loop(1) = {1, 2, 3, 4};\n')	# define line loop
   #fid.write('Plane Surface(1) = {1};\n');	# define surface
   #fid.close()					# close file
   ...
   fid.write('Point(3) = { 1,  1, 0};\n');
   fid.write('Point(4) = {-1,  1, 0};\n');
   fid.write('Point(5) = {-1,  0, 0};\n'); # write points where b.c. changes...
   fid.write('Point(6) = { 1,  0, 0};\n');
   fid.write('Line(1) = {1, 2};\nLine(2) = {2, 6};\nLine(3) = {6, 5};\n');
   fid.write('Line(4) = {5, 1};\nLine(5) = {6, 3};\nLine(6) = {3, 4};\nLine(7) = {4, 5};\n'); # between corners and singular nodes
   fid.write('Line Loop(1) = {1, 2, 3, 4};\n')	# define line loop
   fid.write('Line Loop(2) = {-3, 5, 6, 7};\n')	# define line loop   
   fid.write('Plane Surface(1) = {1};\n');	# define surface
   fid.write('Plane Surface(2) = {2};\n');	# define surface
   fid.close()					# close file
   
   
   subprocess.call(['gmsh', namestring + '.geo', '-2', '-bgm', '../bgmeshes/bgmesh' + bg_str + '.pos', '-o',  namestring + bg_str + '.msh','-v', '3']);	# mesh .geo-file into msh.file,
   p, t, be = read_gmsh(namestring + bg_str + '.msh');			# use read_gmsh to create output
