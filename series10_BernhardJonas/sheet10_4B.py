# Solutions to exercise 4 from sheet 10 in Numerics of PDEs, using method B to generate a
# graded mesh: use background meshes in gmsh
...
# --------------------- Settings ---------------------------------------
J_0=0; # index of most coarse background (bg) mesh to use (bg meshes are grade with exponent beta=4)
energynorm_ref = 1.548888; # reference value for the energy norm of the exact solution (used to check convergence)
J = 6;	# number of meshes to test (k=0..J-1)

# ---------------------- Preparation -----------------------------------
# arrays for mesh properties
bgmeshes = [None]*J;  # will be filled with the bg mesh indices
...
for k in range(0, J):
  ...
  # Mesh generation:
  bgmeshes[k] = J_0+k;	# index of bg mesh to use
  print('using bg mesh #'+str(bgmeshes[k])+':');
  
  # define the mesh using gmsh (corners and singular points are explicitly included as nodes)
  p,t,be = mesh.grid_unstr_sheet10B(bgmeshes[k], 'meshes/10_4B_');	# generate quasi-uniform, unstructured mesh
...
ax1.plot(bgmeshes, nodes,'b o', label='nodes')
ax1.plot(bgmeshes, elements, 'r x', label = 'elements')
ax1.plot(bgmeshes, nodes[0]* 2.**( (np.array(bgmeshes)-J_0) ),'g-', label = '$N\propto 2^k$')
ax1.plot(bgmeshes, nodes[0]* 2.**( (np.array(bgmeshes)-J_0)/2. ),'-', label = '$N\propto 2^{k/2}$')
#ax1.set_xscale('lin')
ax1.set_yscale('log')
ax1.set_xlabel('bg mesh index $k$')
